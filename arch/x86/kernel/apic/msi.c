...
static void dmar_msi_compose_msg(struct irq_data *data, struct msi_msg *msg)
{
	...
	unsigned int cpu = cpumask_first(eff);

	__irq_msi_compose_msg(irqd_cfg(data), msg, true);

	/*
	 * DMAR event interrupts are generated by the IOMMU itself, not by a
	 * PCI function, and are the only interrupt messages that carry a
	 * full 32-bit destination in the upper address register. Deliver
	 * them in physical destination mode: logical (cluster) destinations
	 * outside cluster 0 have been observed to be silently dropped for
	 * IOMMU-generated event messages, which permanently stalls the
	 * page request queue (the event only edges on the PPR 0->1
	 * transition, so one lost interrupt loses all of them).
	 */
	if (cpu < nr_cpu_ids) {
		u32 destid = cpu_physical_id(cpu);

		msg->arch_addr_lo.dest_mode_logical = 0;
		msg->arch_addr_lo.destid_0_7 = destid & 0xFF;
		msg->arch_addr_hi.destid_8_31 = destid >> 8;
	}
}
