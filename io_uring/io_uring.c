...
static __cold void io_ring_exit_work(struct work_struct *work)
{
	...
	unsigned long timeout = jiffies + HZ * 2;// * 5;
...
static __cold int io_uring_create(struct io_ctx_config *config)
{
	 ...
	 * with IORING_SETUP_DMA take one. Unconditional acquisition burned
	 * channels on rings that never DMA (measured: a benchmark's reply-only
	 * client rings consumed half the machine's channels) and made the
	 * grab race the asynchronous channel release of any ring being torn
	 * down, silently degrading the loser to CPU copies. An app that asks
	 * and cannot be served fails loudly here instead.
	 */
