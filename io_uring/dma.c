 ...
 * a folio maps its io_dma_map_quantum()-sized segment DMA_TO_DEVICE and
 * caches {segment-head PFN -> dma_addr} in a per-DSA-device xarray; every
 * later chunk on that segment is pure arithmetic.  dma_unmap leaves the
 * per-I/O path entirely, running only on CLOCK eviction (bytes-capped)
 * and explicit flush.
 *
 ...
 *
 * Why: the standing cache only pays for TEMPORAL reuse (the same folio
 * DMA'd again later).  Streaming sources -- spill files read once and
 * deleted -- get only spatial hits (chunks of one folio during one
 * pass), which per-folio mapping provides with no standing entry at
 * all; their accumulated entries just bloat the IOVA allocator rbtree
 * that every future miss must search (measured: 10.5% of node cycles
 * on 2-node TeraSort at cap 4096, recovered at cap 512).
 *
 * Mechanism: a miss consults a global count-min sketch of per-folio
 * touch frequency.  First-touch folios insert on PROBATION into a small
 * window (io_uring_dma_pfn_cache_window_mb): they serve their pass's
 * spatial hits, then the sweep reclaims them with no second chance, so
 * a pure stream holds only ~window bytes of standing IOVA regardless of
 * cap.  Reuse evidence promotes to main residency: either a hit on a
 * probation entry after IO_PFN_PROBATION_AGE (cross-pass re-touch; the
 * age gate keeps same-pass spatial hits from promoting), or a sketch
 * frequency >= 2 at miss time (the folio came back after its window
 * entry was reclaimed).  Main entries keep the CLOCK second chance
 ...
 * /sys/kernel/debug/io_uring_dma_completion_mode for A/B comparison:
 *
 *  - kworker:  schedule_work() busy-poll on a shared kworker (the original).
 *  - busypoll: per-ctx kthread busy-polls __io_dma_poll().
 *  - mwait:    per-ctx kthread UMONITOR/UMWAITs on the completion record
 *              (added in a later step; selecting it now is rejected).
 *  - irq:      interrupt-driven, no completion thread (later step).
 */
enum {
	IO_DMA_COMP_KWORKER,
	IO_DMA_COMP_BUSYPOLL,
	IO_DMA_COMP_MWAIT,
	IO_DMA_COMP_IRQ,
	IO_DMA_COMP_NR,
};
...
/*
 * Filemap (READ_FIXED page-cache) DMA path accounting: the path falls
 * back to the normal buffered read on ANY failure, silently, so a
 * per-reason count is the only way to see whether it engages at all.
 * Recorded from the io_read gate (io_uring/rw.c).
 */
...
/*
 * Per-ctx completion kthread for the busypoll (and later mwait) modes. Sleeps
 ...
 * NUMA node (required for mwait, harmless for busypoll, good for locality).
			     ...
			     struct iov_iter *from, u64 src_user_addr)
{
	...
	while (written < want && nr_fol < max_fol) {
		...
		for (sub = 0; sub < bytes; ) {
			...
			size_t len = min(bytes - sub, src_remain);
			struct dma_async_tx_descriptor *tx;
...
int io_dma_submit_queued_tasks(struct io_kiocb *req)
{
	...
	if (ret == -EIOCBQUEUED) {
		 ...
		 * completion to a clean context: the per-ctx kthread in busypoll
