...
/*
 * Covered-bytes cap in MiB, where 0 disables the cache entirely.  The
 * default is generous.  Oversizing only widens the exposure window,
 * while a cap under a cycling working set makes the sweep thrash with
 * an evict and remap per I/O, which is worse than no cache.  Size it
 * above the source working set, which is the hot file set for reads.
 */
 ...
 * retry. We count these for visibility in the debugfs latency file.
 */
...
void io_dma_debugfs_init(void)
{
	...
	debugfs_create_u32("io_uring_dma_budget_descs", 0644, NULL,
			   &io_dma_budget_descs);
				   ...
				   unsigned int nr_entries)
{
	...
	/* Capture before the doorbell because dmaengine_submit() makes the
	 * descriptor eligible for a concurrent reap-and-recycle.
	 */
...
collect_done:
	 ...
	 * writes. They fault in the IOMMU instead of landing in reclaimed
	 * memory.
	 */
	for (i = 0; i < nr_fol; i++)
		if (fol[i].dst_dma)
