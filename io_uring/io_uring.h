...
/* Check whether any pollable DMA tasks are in flight.  poll_list
 * belongs to the armed poller, so reading it here is a racy heuristic.
 * That is all the wakeup and wait-loop checks need.
 */
...
/* Filemap DMA-read gate and result accounting. See io_uring/dma.c. */
enum {
	IO_DMA_FM_ENGAGED,
	IO_DMA_FM_SHMEM,
	IO_DMA_FM_NOT_BVEC,
