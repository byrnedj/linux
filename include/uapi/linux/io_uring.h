...
/*
 * Acquire a DMA-engine channel, for example an Intel DSA work queue,
 * for this ring at creation. This enables DMA-offloaded data copies
 * between the page cache and registered buffers. Channels are a finite
 * hardware resource. Without this flag the ring takes none and all
 * copies use the CPU. Ring creation fails with -ENODEV if no channel
 * is available.
 */
